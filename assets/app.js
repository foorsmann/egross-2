...
// Funcționalitate robustă, re-injectare la re-render, accesibilitate, fără duplicare
function initDoubleQtyButtons() {
  // Atașează funcționalitatea la toate butoanele double-qty-btn statice din pagină
  document.querySelectorAll('.double-qty-btn').forEach(btn => {
    // Caută inputul de cantitate din același bloc
    let wrapper = btn.previousElementSibling;
    if (!wrapper || !wrapper.classList.contains('quantity-input')) return;
    const input = wrapper.querySelector('.quantity-input__element');
    if (!input) return;
...
// Funcție helper pentru adăugarea butonului "Dublează" la un input

// Rulează la load și la orice re-render (AJAX, grid, variantă etc.)
document.addEventListener('DOMContentLoaded', initDoubleQtyButtons);
document.addEventListener('shopify:section:load', initDoubleQtyButtons);
document.addEventListener('shopify:section:reorder', initDoubleQtyButtons);
document.addEventListener('shopify:section:unload', initDoubleQtyButtons);
document.addEventListener('shopify:cart:refresh', initDoubleQtyButtons);
// ...adaugă alte evenimente AJAX specifice temei dacă există